module "network_peering_spoke" {
  ...
  resource_network_peer      = var.service_network_spoke[index(values(var.service_location), each.value)]
...
module "network_peering_hub" {
  ...
  service_name               = var.service_name
  service_location           = each.value
  resource_network_peer      = var.service_network_hub[index(values(var.service_location), each.value)]
